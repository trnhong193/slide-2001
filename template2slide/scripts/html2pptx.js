...
function addElements(slideData, targetSlide, pres) {
  for (const el of slideData.elements) {
    if (el.type === 'image') {
      ...
      let videoPath = el.src.startsWith('file://') ? el.src.replace('file://', '') : el.src;
      // Path should already be resolved to absolute in html2pptx function
      // PptxGenJS uses addMedia for videos
      try {
        if (targetSlide.addMedia) {
          targetSlide.addMedia({
            path: videoPath,
            type: 'video',
            x: el.position.x,
            y: el.position.y,
        ...
        } else if (targetSlide.addVideo) {
          targetSlide.addVideo({
            path: videoPath,
            x: el.position.x,
            y: el.position.y,
            w: el.position.w,
          ...
          // Fallback: show warning and skip video
          console.warn(`Video support not available in this version of PptxGenJS. Skipping video: ${videoPath}`);
        }
      } catch (error) {
        console.warn(`Failed to add video ${videoPath}: ${error.message}`);
      }
  ...
  return await page.evaluate(() => {
    ...
    document.querySelectorAll('*').forEach((el) => {
      ...
      if (el.tagName === 'VIDEO') {
        const rect = el.getBoundingClientRect();
        if (rect.width > 0 && rect.height > 0) {
          // Use src attribute which browser resolves, or fallback to data attribute
          const mediaPath = el.src || el.getAttribute('data-media-path');
          if (mediaPath) {
            elements.push({
              type: 'video',
              src: mediaPath,
