...
**REQUIRED FORMAT** (all fields at same level using `**Field:** Value` format):

```
**Project:** [ONE SHORT SENTENCE describing main objective]
...
| Field | Source | Logic |
|-------|--------|-------|
| Project | S1 - "Current Pain Points" | **ONE SHORT SENTENCE** summarizing main objective (e.g., "AI-Powered Video Analytics for Safety Compliance and Incident Prevention"). Extract essence from pain points but keep concise. |
| Project Owner | S1 - "Customer overview" | Customer name. Must use `**Project Owner:**` or `**Client Name:**` format for parser compatibility. |
...
**CRITICAL NOTES:**
- **DO NOT use `###` subheadings** for individual fields - all fields must use `**Field:** Value` format at same level
- **Project field MUST be one sentence** - examples: "AI-Powered Video Analytics for Safety Compliance", "AI-Powered Video Analytics Pilot for Safety Enhancement"
- **Project Owner field** must be extractable by parser (use `**Project Owner:**` or `**Client Name:**`)
